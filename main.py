...
GAME_LOG_SEASONS = ["20252026", "20242025", "20232024", "20222023"]

async def _nhl_player_gamelogs(pid: int, sem: asyncio.Semaphore) -> list:
    """Fetch player game logs from NHL Stats API across multiple seasons."""
    season_filter = " or ".join([f"seasonId={s}" for s in GAME_LOG_SEASONS])
    params = {
        "limit": 300, "start": 0,
        "cayenneExp": f"playerId={pid} and gameTypeId=2 and ({season_filter})"
    }
    async with sem:
        async with httpx.AsyncClient(timeout=20) as c:
            try:
                r = await c.get(f"{NHL_STATS}/skater/gamelog", params=params)
                if r.status_code == 200:
                    return r.json().get("data", [])
            except Exception as e:
                print(f"[NHL] Game log error pid={pid}: {e}")
    return []
...
async def run_picks(target_date: str = None) -> Dict:
    ...
    async def analyze(p: Dict) -> Optional[Dict]:
        ...
        h2, t2, r2 = _calc_hit_rate(logs, line, home_road, opponent=opp)
        h3, t3, r3 = _calc_hit_rate(logs, line, home_road, last_n=10)
...
async function connectSM(){
  ...
  try {
    ...
    if(true){
      ...
      status.innerHTML = '✅ Ready — NHL Stats API + Odds API';
